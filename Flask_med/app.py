...
def save():
    daigonis = request.form['daigonis']
    sym = request.form['symtoms']
    medi = request.form['medi']
    curs=mysql.connection.cursor()
    curs.execute(f"INSERT INTO `prescription` (`email_patient`, `email_doctor`, `Daignois`,`tablets`) VALUES ('{session['patemail']}', '{session['docemail']}', {daigonis},{medi} )")
    curs.connection.commit()
    curs.close()
